<script setup>
    ...
    const show = async (accountId) => {
        ...
        try {
            ...
            if (preferenceStore.getOtpOnRequest && parseInt(preferenceStore.autoCloseTimeout) > 0) {
                startAutoCloseTimer()
            }
        }
        catch(error) {
    ...
    function startAutoCloseTimer() {
        let duration = parseInt(preferenceStore.autoCloseTimeout) // in minutes
...
<template>
    <div>
        ...
        <p v-if="settingStore.hasFeature_showNextOtp && preferenceStore.showNextOtp" class="mt-3 is-size-4">
            ...
            >
                {{ useDisplayablePassword(next_password, preferenceStore.showOtpAsDot && preferenceStore.revealDottedOTP && revealPassword) }}
            </span>
